...
function C:update(data)
	...
	if math.abs(relYawUsed) < deadzone then
		relYawUsed = 0
	end
	if math.abs(relPitchUsed) < deadzone then
		relPitchUsed = 0
	end
	...
	if self.camResetted ~= 1 then
		if self.rollSmoothing > 0.0001 then
			...
			up = (1 / (upSmoothratio + 1) * up + (upSmoothratio / (upSmoothratio + 1)) * self.camLastUp)
			up:normalize()
		else
	...
	local calculatedCamPos = dist
		* vec3(math.sin(rot.x) * math.cos(rot.y), math.cos(rot.x) * math.cos(rot.y), math.sin(rot.y))
	...
	local dir_target = (targetPos - camPos)
