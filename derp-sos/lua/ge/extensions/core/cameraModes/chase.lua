...
function C:reset()
  ...
  ---PHASE 0 is a lightweight wrapper over vanilla camera that the user may use to provide discrete tweaks to orientation, offset, and FOV.
  -- POSITION MODIFIERS
  -- Do users want to set a constant relative offset (eg x -> fov = fov + x) or do they want to set a constant absolute offset? (eg x -> fov = x)
  -- FOV MODIFIERS
  function Constant_user_defined_offset(
    enabled_boolean,
  ...
  function Constant_user_defined_fov_offset(
    ...
    if enabled_boolean == true then
        ...
        offset_contribution_vec3 = additive_fov_offset_scalar
    end
    return offset_contribution_vec3
...
---These modifiers are for "constant" effects like offset/orientation depending on rate of change of some quantity e.g velocity, acceleration
---These must settle back to the mean position of VANILLA when the conditions.
-- POSITION MODIFIER
function Rate_variable_derived_offset(
  enabled_boolean,
  rate_variable_vec3,
  direction_vectors_vec3_vec3,
  coefficients_vec3,
  variable_clamp_ranges_vec3_vec2,
  smoothing_function,
  dt
)
  ...
  local offset_contribution_vec3 = vec3(0, 0, 0)
  if enabled_boolean == true then
      -- Clamping the components of time-derivative variable with the
      -- clamp ranges specified per component
      ...
      if smoothing_function == nil then
      ...
      offset_contribution_vec3 =
        vec3(
          x_leftward_unit_vector_vec3 * coefficients_vec3.x * smoothing_function(rate_variable_vec3.x, dt) +
          y_backward_unit_vector_vec3 * coefficients_vec3.y * smoothing_function(rate_variable_vec3.y, dt) +
          z_upward_unit_vector_vec3   * coefficients_vec3.z * smoothing_function(rate_variable_vec3.z, dt)
      )
  end
  return offset_contribution_vec3
...
function Rate_variable_derived_fov_offset(
  ...
  if enabled_boolean == true then
      ...
      --Apply our offset calculations to our piece of the offset pie.
      -- We are responsible only for cooking this relative value'd vec3 and when we return it we do not care what else happens
      fov_offset_contribution_scalar = coefficient_scalar * smoothing_function(rate_variable_magnitude, data.dt)
  end
...
function C:update(data)
  ...
  self.relative_Yaw_degreesBydegrees =   clamp(self.relative_Yaw_degreesBydegrees   + 0.15*MoveManager.yawRelative  , -1, 1)
  self.relative_Pitch_degreesBydegrees = clamp(self.relative_Pitch_degreesBydegrees + 0.15*MoveManager.pitchRelative, -1, 1)
  ...
  local nx_leftwardfacing_directionvector_relativeto_carorigin = left_vec3_point_relativeto_carorigin - ref_vec3_point_carorigin_relativeto_carorigin
  local ny_backwardfacing_directionvector_relativeto_carorigin = back_vec3_point_relativeto_carorigin - ref_vec3_point_carorigin_relativeto_carorigin
  ...
  local nz_upwardfacing_directionvector_relativeto_carorigin = nx_leftwardfacing_directionvector_relativeto_carorigin:cross(ny_backwardfacing_directionvector_relativeto_carorigin):normalized()
  ...
  if self.offset_vec3_metres_relativeto_what and self.offset_vec3_metres_relativeto_what.x then
    ...
    self.camera_base_location_afteroffset_vec3_metres_relativeto_what:set(self.offset_vec3_metres_relativeto_what.x / (nx_leftwardfacing_directionvector_relativeto_carorigin:length() + 1e-30), self.offset_vec3_metres_relativeto_what.y / (ny_backwardfacing_directionvector_relativeto_carorigin:length() + 1e-30), self.offset_vec3_metres_relativeto_what.z / (nz_upwardfacing_directionvector_relativeto_carorigin:length() + 1e-30))
  else
  ...
  --wtf is the point or use of the below if-else branch?
    ...
    local camera_base_location_2_afteroffset_vec3_metres_relativeto_absolute = nx_leftwardfacing_directionvector_relativeto_carorigin * self.camera_base_location_afteroffset_vec3_metres_relativeto_what.x + ny_backwardfacing_directionvector_relativeto_carorigin * self.camera_base_location_afteroffset_vec3_metres_relativeto_what.y + nz_upwardfacing_directionvector_relativeto_carorigin * self.camera_base_location_afteroffset_vec3_metres_relativeto_what.z
    target_position_vec3_metres_relativeto_absolute = data.pos + ref_vec3_point_carorigin_relativeto_carorigin + camera_base_location_2_afteroffset_vec3_metres_relativeto_absolute
  end

  -- CAR UNIT VECTOR FACING BACKWARD
  local dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin = (ref_vec3_point_carorigin_relativeto_carorigin - back_vec3_point_relativeto_carorigin); dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin:normalize()
  ...
  local up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin = dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin:cross(left_vec3_point_relativeto_carorigin); up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin:normalize()
  ...
  if self.camResetted ~= 1 then
    if self.rollSmoothing > 0.0001 then
      ...
      up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin = (1 / (upSmoothratio + 1) * up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin + (upSmoothratio / (upSmoothratio + 1)) * self.camLastUp); up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin:normalize()
    else
    ...
    dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin:set(self.dirSmoothX:getUncapped(dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin.x, data.dt*1000), self.dirSmoothY:getUncapped(dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin.y, data.dt*1000), self.dirSmoothZ:getUncapped(dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin.z, data.dt*1000)); dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin:normalize()
  end
  ...
  rotation_euler3_radians.x = 1 / (ratio + 1) * rotation_euler3_radians.x + (ratio / (ratio + 1)) * self.camera_previous_frame_rotation_radians.x
  rotation_euler3_radians.y = 1 / (ratio + 1) * rotation_euler3_radians.y + (ratio / (ratio + 1)) * self.camera_previous_frame_rotation_radians.y

  -- What the hell?
  -- Linear interpolating between current and last frame's distance values
  local dist = 1 / (ratio + 1) * self.cameradistance_scalar_metre + (ratio / (ratio + 1)) * self.camera_previous_frame_distance_metres
  ...
  local quaternion_direction_of_heading = quatFromDir(-dir_vec3_metres_backwardfacing_car_unitvector_relativeto_carorigin, up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin)
  -- Cook the calculatedCameraPosition with the quaternion rotation sauce. Idk why?
  calculatedCameraPosition_point_vec3_metres_relativeto_what = quaternion_direction_of_heading * calculatedCameraPosition_point_vec3_metres_relativeto_what

  -- What the hell?
  local finalvanilla_camera_position_vec3_metres_relativeto_absolute = calculatedCameraPosition_point_vec3_metres_relativeto_what + target_position_vec3_metres_relativeto_absolute

  -- The line drawn from the fulcrum to the final camera position
  local dir_target_to_camera_vec3_metres_direction_vector = (target_position_vec3_metres_relativeto_absolute - finalvanilla_camera_position_vec3_metres_relativeto_absolute); dir_target_to_camera_vec3_metres_direction_vector:normalize()
  -- Funny quaternion rotation recipe of cam-target vec about the upward-carvec as the axis of rotation
  local finalvanilla_camera_rotation_quaternion = quatFromDir(dir_target_to_camera_vec3_metres_direction_vector, up_vec3_metres_upwardfacing_car_unitvector_relativeto_carorigin)
  ...
  --- TO IMPLEMENT
  -- velocity, acceleration refers purely to VEHICLE's own velocity/acceleration. we dont fuck with camera acceleration yet
  -- cardir: using    car's direction vectors
  -- camdir: using camera's direction vectors
  ...
  -- TODO: Can vel/acc based functions be combined into one that handles both cases with branches?
  ...
  -- PHASE 0 reverts back to vanilla_camera if the user doesn't change anything in it.

  function constant_user_defined_orientation(enabled, ...) end        -- J, N. Implement separately. INVESTIGATE!!


  --ORIENTATION MODIFIERS
  function     velocity_derived_orientation(enabled, car_velocity)
    if enabled == true then
      car_velocity = car_velocity or data
    end         -- J, N. Implement separately. INVESTIGATE!!
  end
  function acceleration_derived_orientation(enabled, ...) end         -- J, N. Implement separately. INVESTIGATE!!

  ---PERIODIC MODIFIERS, PHASE 2
  ---These must settle back to the mean position of PHASE 1 when their own conditions are no longer satisfied.
  -- POSITION MODIFIERS
  function     velocity_derived_offset_shake_cardir(enabled, car_velocity) end
  function acceleration_derived_offset_shake_cardir(enabled, ...) end -- O

  function     velocity_derived_offset_shake_camdir(enabled, car_velocity) end
  function acceleration_derived_offset_shake_camdir(enabled, ...) end -- K

  --ORIENTATION MODIFIERS
  function     velocity_derived_orientation(enabled, car_velocity)
    if enabled == true then
      car_velocity = car_velocity or data end
    end         -- J, N. Implement separately. INVESTIGATE!!
  function acceleration_derived_orientation(enabled, ...) end         -- J, N. Implement separately. INVESTIGATE!!

  ---COLLISION MODIFIERS, PHASE 3
  ---This is a special modifier phase that is applied only in times of collision, i.e extreme transfer of momentum/force upon collision with another car or terrain.
  --Implement Graze collision detection (see design document: The Matter of Collision Based Effects)
  function collision_derived_offset_springdampenedshake_cardir(enabled, ...) end -- idk collision? O? not sure
  function collision_derived_offset_springdampenedshake_camdir(enabled, ...) end -- idk collision? K? not sure

  function collision_derived_orientation_springdampenedshake(enabled, ...) end -- idk collision? J,N? not sure

  function dolly_zoom(enabled, ...) end                               -- K (I think) (Where should this go? If user wants Dolly Zoom, it Must somehow override any other position/FOV settings.)

  -- These to be summed/added to the ground state position/rotation/FOV.
  local final_offset_vec3-- =
  --Rate_variable_derived_offset{enabled_boolean=true, rate_variable_vec3=data.vel, direction_vectors_vec3_vec3=camera_direction_vectors} +
  --Rate_variable_derived_offset{enabled_boolean=true, rate_variable_vec3=data.acc, direction_vectors_vec3_vec3=   car_direction_vectors}

  local final_fov_offset_scalar-- =
  --Rate_variable_derived_offset{enabled_boolean=true, rate_variable_vec3=data.vel} +
  --Rate_variable_derived_offset{enabled_boolean=true, rate_variable_vec3=data.acc}

  local final_orientation-- =
  ...
  data.res.rot = ground_state_camera_rotation * final_orientation
